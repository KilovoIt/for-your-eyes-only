...
def coordinatesPrint(tags):
    ...
    assert all([isinstance(tags[x], exifread.classes.IfdTag) for x in                                 \
        ['GPS GPSLatitude', 'GPS GPSLongitude']])        \
            == True
    try:
        print_lat = str(tags['GPS GPSLatitude'].values[0])+ '° ' +                         \
            str(tags['GPS GPSLatitude'].values[1]) + "' " +                               \
            str(tags['GPS GPSLatitude'].values[2]) + '"' +                                \
            str(tags['GPS GPSLatitudeRef'].values)

        print_lon = str(tags['GPS GPSLongitude'].values[0])+ '° ' +                         \
            str(tags['GPS GPSLongitude'].values[1]) + "' " +                               \
            str(tags['GPS GPSLongitude'].values[2]) + '"' +                                \
            str(tags['GPS GPSLongitudeRef'].values)
...
def coordinatesReformatting(tags):
    ...
    string_lat = float(tags['GPS GPSLatitude'].values[0]) +                         \
        float(tags['GPS GPSLatitude'].values[1])/60 +                               \
        float(tags['GPS GPSLatitude'].values[2])/3600

    string_lon = float(tags['GPS GPSLongitude'].values[0]) +                        \
...
data_of_interest = {'Date':'Image DateTime', 'Make':'Image Make', 'Model':'Image Model'}

if not tags.values(): # Case when there is no EXIF date
    ...
    for param in set(tags.keys()) & set(data_of_interest.values()):
        output.append(str(list(data_of_interest.keys())[list(data_of_interest.values()).index(param)])+\
            ': ' + str(tags[param]))

    #Dealing with geodata format
    try:
        if all([tags[tag] for tag in ['GPS GPSLatitude', 'GPS GPSLongitude', 'GPS GPSLatitudeRef', 'GPS GPSLongitudeRef']]) == True:
            gps_avail = True
            Lat_print, Lon_print = coordinatesPrint(tags)
            output.extend(['Latitude:  '+str(Lat_print), 'Longitude:  '+str(Lon_print)])

        elif all([tags[tag] for tag in ['GPS GPSLatitude', 'GPS GPSLongitude']]) == True:
    ...
    if gps_avail:
        ...
        if value == 'Y' or value == 'y':
            webbrowser.open('https://www.google.com/maps/place/' + place_string[0]+' '+place_string[1])
